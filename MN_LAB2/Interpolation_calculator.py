...
for line in lines:
    ...
    if time == ' Nan':
        nan_count += 1
        print(f"{nan_count}st NaN:")
        print()
        time = newton_interp(x1[-10:], y1[-10:], days - 1)
        rounded_number = format(time, ".1f")
        print(f"{rounded_number} (Newton's interpolation)")
        time = lagrange_interp(x1, y1, days - 1)
        print(f"{time} (Lagrange interpolation)")
        time = piecewise_linear_interp(x1, y1, days - 1)
        print(f"{time} (Piecewise linear interpolation)")
        time = cubic_spline_interp(x1, y1, days - 1)
        print(f"{time} (Cubic spline interpolation)")
